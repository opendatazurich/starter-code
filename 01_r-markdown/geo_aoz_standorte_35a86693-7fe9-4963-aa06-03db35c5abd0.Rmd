---
title: "Open Government Data, OpenDataZurich"
date: "2025-02-19"
output: html_document
---

## Dataset: AOZ Standorte

AOZ (Asyl Organisation Zürich) - Standorte  

**Zweck**: Visualisierung der Standorte in einer interaktiven Karte im Internet.  

**Genereller Hinweis zum Geodatensatz:**


Weitere Informationen zu diesem Datensatz finden sie unter [«**AOZ Standorte»** auf Geocat.ch](https://www.geocat.ch/geonetwork/srv/ger/catalog.search#/metadata/884729c8-8872-473e-8816-dba4b1bf454d).				
			   

**Statisches Vorschaubild:**

![BildText](https://www.gis.stadt-zuerich.ch/zueriplan_docs/geocat/884729c8-8872-473e-8816-dba4b1bf454d.png)



[Direct link by **OpenDataZurich** for dataset](https://data.stadt-zuerich.ch/dataset/geo_aoz_standorte)<br>
{{ DATASHOP_LINK_ORGANIZATION }}

Auto generated R starter code for data set geo_aoz_standorte.

## Metadata

- **Publisher** `AOZ / DIR / KOM, Asyl Organisation Zürich, Asyl Organisation Zürich`
- **Maintainer** `Open Data Zürich`
- **Maintainer_email** `opendata@zuerich.ch`
- **Keywords** `Bevölk­erung`
- **Tags** `['asyl', 'geodaten', 'geoportal', 'punktdaten', 'stzh', 'vektordaten']`
- **Metadata_created** `2023-11-06T03:16:30.024447`
- **Metadata_modified** `2025-02-19T04:36:10.694333`


# Load packages

```{r}

library(tidyverse)
library(skimr)
library(sf)

```

# Load data

-   The dataset has {{ DISTRIBUTION_COUNT }} distribution(s) in CSV format.
-   All available CSV distributions are listed below and can be read into a tibble.

```{r}

# Helper function to get available layers from WFS
get_wfs_layers <- function(url) {
  dataset_identifier <- sub(".*/(.*)\\?.*", "\\1", url)
  url_geoportal <- paste0("https://www.ogd.stadt-zuerich.ch/wfs/geoportal/", dataset_identifier)
  
  cat("Getting available layers from:", url_geoportal, "\n")
  
  params <- list(service = "WFS", version = "1.1.0", request = "GetCapabilities")
  
  response <- httr::GET(url_geoportal, query = params)
  content <- httr::content(response, as = "text", encoding = "UTF-8")
  
  layers <- xml2::read_xml(content) %>%
    xml2::xml_find_all(".//wfs:FeatureType/wfs:Name", xml2::xml_ns(content)) %>%
    xml2::xml_text()
  
  cat("Available layers:", paste(layers, collapse = ", "), "\n")
  
  return(layers)
}

# Fetch available layers
layers <- get_wfs_layers("# name: 		AOZ_Standorte.json (GeoJSON-Services)
# filename: 		nan
# format: 		JSON
# url: 		https://www.stadt-zuerich.ch/geodaten/download/AOZ_Standorte?format=geojson_link
# id: 		35a86693-7fe9-4963-aa06-03db35c5abd0
# resource_type: 		api
# package_id: 		d3fa118e-b12c-44bf-9337-ee4e767420d4

library(readr) 
df <- read_csv("https://www.stadt-zuerich.ch/geodaten/download/AOZ_Standorte?format=geojson_link")")

# Set first layer as default
typename <- layers[1]
cat("Chosen typename:", typename, "\n")

# Construct WFS request URL
wfs_url <- paste0("# name: 		AOZ_Standorte.json (GeoJSON-Services)
# filename: 		nan
# format: 		JSON
# url: 		https://www.stadt-zuerich.ch/geodaten/download/AOZ_Standorte?format=geojson_link
# id: 		35a86693-7fe9-4963-aa06-03db35c5abd0
# resource_type: 		api
# package_id: 		d3fa118e-b12c-44bf-9337-ee4e767420d4

library(readr) 
df <- read_csv("https://www.stadt-zuerich.ch/geodaten/download/AOZ_Standorte?format=geojson_link")&service=WFS&version=1.1.0&request=GetFeature&typename=", typename, "&outputFormat=application/json")

# Load GeoJSON data
df <- st_read(wfs_url)

```

# Analyze data

```{r}

glimpse(df)
str(df)
skim(df)

head(df)
tail(df)

# Drop empty columns
df <- df %>% select(where(~!all(is.na(.))))

# Print dataset info
cat(sprintf("The dataset has %d rows (observations) and %d columns (variables).\n", nrow(df), ncol(df)))
cat(sprintf("There seem to be %d exact duplicates in the data.\n", sum(duplicated(df))))

# Memory usage
tibble::glimpse(df)
```

# Plot data

```{r}
plot(df$geometry)
```
# Continue your code here...

```{r}

```

------------------------------------------------------------------------

# Contact

opendata@zuerich.ch
